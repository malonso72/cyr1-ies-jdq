// Comprobación de las páginas de reto de T2 · micro:bit (calcada de test_sesiones.js de T1).
// Carga cada reto (r01…r15, a01…a12, m01…m03) en jsdom sin red y verifica estructura, accesibilidad
...
const RAIZ = path.resolve(__dirname, '..', '..', 'trimestres', 't2-microbit', 'retos') +
             path.sep;
...
(async () => {
  // sin argumentos: todas las páginas; con argumentos: r03 a10 m01 …
  let nombres = process.argv.slice(2);
  if (!nombres.length) nombres = fs.readdirSync(RAIZ).filter(f => /^[ram]\d\d\.html$/.test(f)).map(f => f.slice(0, 3)).sort();
  for (const nn of nombres) {
    const n = Number(nn.slice(1));
    const abs = RAIZ + nn + '.html';
    const errs = [];
    ...
    ok(svgs.length > 0 || nn[0] === 'm', P + 'no hay ningún SVG de bloques');
    svgs.forEach((s, i) => {
    ...
    ok(/Moodle/.test(d.body.textContent) || nn[0] === 'm', P + 'no menciona Moodle');
    ok(!!d.querySelector('.entrega'), P + 'falta el bloque de entrega');
    ok(q('a[href^="r"],a[href^="a"],a[href^="m"]').length >= 1, P + 'sin enlaces a retos vecinos');
    ok(!/\.hex/.test(d.body.textContent) || /microbit-/.test(d.body.textContent) || nn[0] === 'm', P + 'la entrega no dice el nombre del archivo');
    dom.window.close();
    console.log('  ' + nn + ' revisada');
  }
