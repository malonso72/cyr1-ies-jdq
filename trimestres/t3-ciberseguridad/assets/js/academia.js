...
const Academia = {
  ...
  // ── VALIDACIÓN DE RESPUESTAS DEL INFORME ─────────────────────
  // Rechaza tokens espurios tipo "f f f f f f f f" o "a a a a a a a a".
  // Una respuesta válida debe tener: al menos N palabras DE 2+ CARACTERES
  // y al menos M letras alfabéticas totales.
  respuestaInformeValida(texto, minPalabras = 8, minLetras = 25) {
    const t = (texto || '').trim();
    const tokens = t.match(/\S+/g) || [];
    const palabras = tokens.filter(x => x.length >= 2).length;
    const letras = (t.match(/[a-záéíóúñüA-ZÁÉÍÓÚÑÜ]/g) || []).length;
    if (palabras < minPalabras) {
      return { ok: false, motivo: `solo ${palabras} palabras válidas (mínimo ${minPalabras}, sin contar letras sueltas)` };
    }
    if (letras < minLetras) {
      return { ok: false, motivo: `solo ${letras} letras escritas (mínimo ${minLetras})` };
    }
    return { ok: true };
  },
